...
describe("Defaults included", () => {
	...
	it("Didn't copy molecule/default/tests files", () => {
		...
		assert.noFile(
			`${prompts.roleName}/molecule/default/tests/test_default.pyc`
		);
	});
	...
	it("Contents of README.md", () => {
		...
		assert.fileContent(
			`${prompts.roleName}/README.md`,
			`Author: ${prompts.gitAuthorName}`
		);
		assert.fileContent(
			`${prompts.roleName}/README.md`,
			`Email: ${prompts.gitAuthorEmail}`
		);
	});
  ...
  it("Permissions of run-test.sh", () => {
	...
	it("Content package.json", () => {
		assert.JSONFileContent(
			`${prompts.roleName}/package.json`,
			buildPkgJson(prompts)
		);
	});
});
...
describe("CiCd", () => {
	describe("None", () => {
		...
		it("No ci file present", () => {
			assert.noFile(`${prompts.roleName}/.travis-ci.yml`);
			assert.noFile(`${prompts.roleName}/.circle-ci/config.yml`);
		});
	});
    ...
    const prompts = {
		...
		it("Travis ci present", () => {
			assert.file(`${prompts.roleName}/.travis.yml`);
		});
	});
});
...
describe("Meta included", () => {
	const prompts = {
		...defaultPrompts,
		...{ includeMeta: true, metaCompany: "Company co." }
	};
	...
	it("Contents meta/main.yml", () => {
		assert.fileContent(
			`${prompts.roleName}/meta/main.yml`,
			`author: ${prompts.gitAuthorName}`
		);
		assert.fileContent(
			`${prompts.roleName}/meta/main.yml`,
